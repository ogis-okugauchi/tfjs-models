...
const videoWidth = params.w || params.width || 640; // 600;
const videoHeight = params.h || params.height || 360; // 500;
...
function setupGui(cameras, net) {
    ...
    // person to be in the frame or results will be innaccurate.
    ...
    // Input resolution:  Internally, this parameter affects the height
    // and width of the layers in the neural network. The higher the value
    // of the input resolution the better the accuracy but slower the speed.
    ...
    // width of the layers in the neural network. The lower the value of
    // the output stride the higher the accuracy but slower the speed,
    ...
    // model. The available options are 1 byte, 2 bytes, and 4 bytes.
    // The higher the value, the larger the model size and thus the longer
    // the loading time, the lower the value, the shorter the loading time
...
function detectPoseInRealTime(video, net) {
    ...
    // If instead we flip the image, then correcting left-right keypoint
    // pairs requires a permutation on all the keypoints.
